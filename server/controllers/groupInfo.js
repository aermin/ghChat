 ...
 * @return
 */
const isInGroup = async (ctx, next) => {
...
const createGroup = async (ctx, next) => {
  ...
  const arr = [uuid, ctx.request.body.name, ctx.request.body.group_notice, ctx.request.body.avatar, ctx.name, ctx.request.body.create_time];
